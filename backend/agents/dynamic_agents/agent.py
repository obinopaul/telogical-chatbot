...
from backend.agents.dynamic_agents.tools import (transfer_to_reflection_agent, transfer_to_main_agent, math_counting_tool,
...
from backend.agents.dynamic_agents.prompts import REFLECTION_PROMPT, MAIN_PROMPT, CONTEXTUALIZER_SYSTEM_PROMPT
...
#     """
#     Analyzes the latest user query and generates contextual bullet points to aid understanding.
#     """
#     # print("--- Entering Contextualize Query Node ---") # Optional debug
    
#     # Convert messages from state for processing
#     processed_history_messages: List[BaseMessage] = []
#     for msg_data in state.get("messages", []):
#         converted_msg = _convert_to_base_message(msg_data) # Assuming _convert_to_base_message is defined
#         if converted_msg:
#             processed_history_messages.append(converted_msg)

#     latest_user_query_content: Optional[str] = None
    
#     # Find the last HumanMessage to get the user's most recent query
#     for msg in reversed(processed_history_messages):
#         if isinstance(msg, HumanMessage):
#             # Assuming _extract_string_content_from_message handles complex HumanMessage content
#             latest_user_query_content = _extract_string_content_from_message(msg)
#             break
            
#     if not latest_user_query_content or not latest_user_query_content.strip():
#         # print("Contextualize Node: No user query found or query is empty. Skipping.") # Optional debug
#         return {"messages": []} # No new messages to add if no query

#     # Prepare the prompt for the contextualizer LLM
#     prompt_for_context_llm = ChatPromptTemplate.from_messages(
#         [("human", CONTEXTUALIZER_SYSTEM_PROMPT.format(user_query=latest_user_query_content))]
#     )
    
#     secondary_llm = get_telogical_secondary_llm()
#     context_llm_chain = prompt_for_context_llm | secondary_llm # Using secondary_llm for this focused task

#     try:
#         ai_response = await context_llm_chain.ainvoke({})
#         generated_bullet_points = ai_response.content.strip()
#         print(f"Contextualize Node: Generated bullet points:\n{generated_bullet_points}") # Optional debug
#     except Exception as e:
#         print(f"Error in contextualizer LLM call: {e}")
#         return {"messages": []} # Don't add a message if context generation fails

#     if not generated_bullet_points:
#         # print("Contextualize Node: LLM returned empty bullet points. Skipping.") # Optional debug
#         return {"messages": []}

#     # Construct the SystemMessage containing the original query and the generated context
#     contextual_message_content = (
#         f"Original User Query:\n\"{latest_user_query_content}\"\n\n"
#         f"Contextual Insights (for AI assistant to consider):\n{generated_bullet_points}"
#     )
    
#     context_system_message = SystemMessage(content=contextual_message_content)
#     # print("--- Exiting Contextualize Query Node ---") # Optional debug
    
#     # This message will be added to the overall message history by the graph
#     return {"messages": [context_system_message]}
...
async def refine_output_refined(state: RefinedAgentState) -> Dict[str, Any]:
    ...
    You are the **Refinement Specialist** for Telogical Systems' AI assistant. Your designated role is to meticulously process the output generated by the primary AI analysis component (`Agent Output to Refine`), ensuring that every user-facing response is impeccably polished, professional, and authentically represents the voice and high standards of Telogical Systems LLC. Consider yourself the final quality assurance step, transforming detailed analytical output into a refined, client-ready communication that directly reflects Telogical's expertise.
    
    Your primary task is to **refine** the provided `Agent Output to Refine`. This means you must **remove specific unwanted elements** while **preserving the integrity, detail, and factual substance of the original information**. **Crucially, all refinements must ensure the final output remains a direct, relevant, and coherent answer to the `Original Query` that prompted the agent's output.** The goal is to produce responses that present information as Telogical's official product, appropriately addressing the user's `Original Query`, not as an AI searching a database or narrating its thought process.

    **Key Refinement Guidelines:**

    1.  **Client Priority: Preserve All Core Information, Detail, and Original Formatting Faithfully:**
        * Your paramount goal is to present the original information with **absolute accuracy and completeness**. Telogical's clients prioritize rich, detailed, and meticulously structured data over brevity or summarization. They expect the full depth of data available.
        * **ABSOLUTELY DO NOT** summarize, paraphrase, re-interpret, condense, or otherwise alter the core data, factual details, or the substance of the information provided in the agent's output. Your task is **not** to "improve" or "re-word" the factual content if it is already clear and accurate; your role is to ensure it is presented cleanly as if from Telogical Systems directly. Any attempt to "make it more product-friendly" by reducing detail is counter to the client's needs.
        * **MAINTAIN THE ORIGINAL FORMATTING AND LEVEL OF DETAIL WITH EXTREME FIDELITY, ESPECIALLY FOR TABLES, LISTS, AND ALL FORMS OF STRUCTURED DATA.** If the original output includes tables, lists, code blocks, specific paragraph structures, or any other structured formatting containing factual information, these structures **MUST be replicated exactly as they were**, minus only the AI thought process elements. For example, if a table is provided, reproduce the table in its entirety and its original structure; **do not** convert it into a prose summary or alter its presentation in a way that reduces detail.
    ...
    4.  **Advanced Scrutiny Based on Tool Outputs (If Provided):**
        * You will be given the `Agent Output to Refine` and may also receive `Supporting Tool Outputs from Agent's Process`. These are the results the previous agent received from any tools it used.
        * Your task includes critically examining these tool outputs in conjunction with the `Agent Output to Refine`.
        * Assess:
            * Does the `Agent Output to Refine` accurately and fully reflect the information available in the `Supporting Tool Outputs`?
            * Did the agent interpret the tool outputs correctly?
            * Are there any discrepancies, omissions, or misinterpretations in the `Agent Output to Refine` when compared against the tool data?
            * Could the information from the tools be leveraged more effectively or presented more clearly?
        * **Your Goal:** If you identify areas where the `Agent Output to Refine` can be improved or corrected based on the `Supporting Tool Outputs`, incorporate these enhancements into your refined response. This may involve correcting inaccuracies, adding missed details *that are explicitly present in the tool outputs*, or clarifying information, all while adhering to guidelines 1-3.
        * **If No Improvements Needed:** If the `Agent Output to Refine` already makes excellent and accurate use of the tool information, or if no tool outputs were provided/relevant, then proceed with the standard refinement tasks (guidelines 1-3). You are not required to invent changes if the output is sound. The refined output should then be the original agent output, subjected only to the cleaning rules in guidelines 1-3.
        * **Constraint:** You are **not** to re-run any tools or seek new information. Your analysis is strictly limited to the provided `Agent Output to Refine` and the `Supporting Tool Outputs`.
